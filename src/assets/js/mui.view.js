 ...
 * @param {Object} window
 */
var CLASS_LEFT = mui.className('left');
var CLASS_CENTER = mui.className('center');
var CLASS_RIGHT = mui.className('right');
...
var CLASS_NAVBAR_LEFT = mui.className('navbar-left');
var CLASS_NAVBAR_CENTER = mui.className('navbar-center');
...
var SELECTOR_ICON = mui.classSelector('.icon');
var SELECTOR_NAVBAR = mui.classSelector('.navbar');
var SELECTOR_NAVBAR_INNER = mui.classSelector('.navbar-inner');
var SELECTOR_PAGES = mui.classSelector('.pages');
var SELECTOR_BTN_NAV = mui.classSelector('.btn-nav');
...
var View = mui.Class.extend({
    ...
    _webkitTransitionEnd: function(event) {
        ...
        var self = this;
        if (mui.os.ios && this.options.animateNavbar && this.previousNavElements && this.activeNavElements) {
            var isBack = this.isBack;
            mui.each(this.previousNavElements, function(i, el) {
                el.classList.remove(CLASS_TRANSITIONING);
                isBack && self._cleanStyle(el);
            });
            mui.each(this.activeNavElements, function(i, el) {
                el.classList.remove(CLASS_TRANSITIONING);
                self._cleanStyle(el);
            });
    ...
    _trigger: function(eventType, page) {
        ...
        mui.trigger(this.view, eventType, {
            page: page
        });
    },
    ...
    _drag: function(event) {
        ...
        if (this.dragging) {
            ...
            if (!this.moved) { //start
                ...
                mui.gestures.touch.startDirection = detail.direction;
            } else { //move
...
mui.fn.view = function(options) {
    var self = this[0];
    var viewApi = null;
    var id = self.getAttribute('data-view');
    if (!id) {
        id = ++mui.uuid;
        mui.data[id] = viewApi = new View(self, options);
    } else {
        viewApi = mui.data[id];
    }
    return viewApi;
